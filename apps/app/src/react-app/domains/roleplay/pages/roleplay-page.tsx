...
import type { RoleplayCharacterRecord, RoleplayPersona, RoleplayPersonaRecord } from "@openwork/types/roleplay";
...
export function RoleplayPage({ endpoint }: RoleplayPageProps) {
  ...
  const [editing, setEditing] = React.useState<RoleplayCharacterRecord | null>(null);

  const personaRecord: RoleplayPersonaRecord = React.useMemo(
    () => personas.data?.[0] ?? createBlankPersona(createPersonaId(Date.now(), randomSuffix()), Date.now()),
    [personas.data],
  );
  ...
  const persist = (character: RoleplayCharacterRecord, message: string) => {
    saveCharacter.mutate(character, {
      ...
      onError: (error: unknown) => toast.error(error instanceof Error ? error.message : "Could not save the character"),
    });
  };

  if (!endpoint) {
    return <p className="text-muted-foreground text-sm">Select a workspace to manage roleplay characters.</p>;
  }
    ...
    <div className="flex flex-col gap-6">
      <CharacterList
        ...
        onDelete={(character) => {
          deleteCharacter.mutate(character.id, {
            ...
            onError: (error: unknown) => toast.error(error instanceof Error ? error.message : "Could not delete the character"),
          });
        }}
      />
      ...
      <section className="flex flex-col gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Who you play. Characters use this to address you.</p>
        </div>
        <PersonaEditor
          ...
          onSave={(persona) =>
            savePersona.mutate(persona, {
              ...
              onError: (error: unknown) => toast.error(error instanceof Error ? error.message : "Could not save the persona"),
            })
          }
        />
      </section>
