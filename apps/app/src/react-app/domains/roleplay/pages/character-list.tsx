    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyDescription>Create a character to start a roleplay conversation.</EmptyDescription>
        </EmptyHeader>
    ...
    <div className="flex flex-col gap-3">
      ...
      <ul className="flex flex-col gap-2">
        {characters.map((character) => (
          <li key={character.id} className="border-border flex items-center gap-3 rounded-md border p-3">
            <button type="button" className="flex-1 text-left" onClick={() => onOpen(character)}>
              <p className="text-sm font-medium">{character.card.data.name || "Untitled character"}</p>
