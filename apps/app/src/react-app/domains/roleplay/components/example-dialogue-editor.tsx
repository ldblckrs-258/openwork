...
import { joinExampleMessages, splitExampleMessages } from "@/app/roleplay/macros";
 ...
 */
export function ExampleDialogueEditor({ value, onChange }: ExampleDialogueEditorProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <Button type="button" variant="outline" size="sm" onClick={() => commit([...items, ""])}>
          <Plus className="size-4" />
          Add exchange
        </Button>
      </div>
      <p className="text-muted-foreground text-sm">
        Short sample exchanges that show how the character speaks. Use <code>{"{{char}}"}</code> and{" "}
        <code>{"{{user}}"}</code> for the names.
      </p>
