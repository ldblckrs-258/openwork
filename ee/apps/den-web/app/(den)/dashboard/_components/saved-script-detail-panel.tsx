...
function initialFields(detail: SavedScriptDetail): Fields {
  return { name: detail.title, description: detail.description ?? "", code: detail.currentVersion.code, input: "{}", inputSchema: pretty(detail.currentVersion.inputSchema), outputSchema: pretty(detail.currentVersion.outputSchema) };
}
    ...
    <div className="space-y-5" data-testid="den-saved-script-detail">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <button type="button" aria-label="Back to plugin" onClick={close} className="mt-0.5 rounded-lg p-2 text-gray-400 hover:bg-gray-50 hover:text-gray-800"><ArrowLeft className="h-4 w-4" /></button>
          <div><div className="flex flex-wrap items-center gap-2"><h1 className="text-[18px] font-semibold text-gray-950">{detail.title}</h1><span className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] text-gray-500">Version {detail.currentVersion.id.slice(0, 8)}</span><span className={`rounded-full px-2 py-0.5 text-[11px] ${detail.freshness.state === "needs_attention" ? "bg-red-50 text-red-600" : detail.freshness.state === "fresh" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{detail.freshness.state.replace("_", " ")}</span></div><p className="mt-1 text-[13px] text-gray-400">Durable, editable, refreshable Dynamic Artifact · {currentAutomationCount} pinned Automation{currentAutomationCount === 1 ? "" : "s"}</p></div>
        </div>
        <div className="flex flex-wrap gap-2"><label className="flex items-center gap-2 text-[12px] text-gray-400">Stale after<select value={maxAgeMs} onChange={(event) => setMaxAgeMs(Number(event.currentTarget.value))} className="h-9 rounded-xl border border-gray-200 bg-white px-2 text-[13px] text-gray-700">{AGE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><DenButton disabled={pending} onClick={() => { setLocalError(null); void runMutation.mutateAsync({ pluginId: detail.pluginId, configObjectVersionId: detail.currentVersion.id, input: parseJson("Example input", fields.input) }).catch((reason) => setLocalError(message(reason))); }}><RefreshCw className="h-3.5 w-3.5" />Refresh now</DenButton></div>
      </div>
